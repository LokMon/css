...
<template>
  <div class="grid-container">
    <section class="property-value">
      ...
      <div class="value-set">
        <div class="value-item" v-for="(item, index) in gridTemplateColumns" :key="index">
          ...
          <el-button
            class="btn"
            v-if="gridTemplateColumns.length != 1"
            @click="delColum(index)"
            plain
            size="mini"
            icon="el-icon-minus"
            circle
    ...
    <section class="property-value">
      ...
      <div class="value-set">
        <div class="value-item" v-for="(item, index) in gridTemplateRows" :key="index">
          ...
          <el-button
            class="btn"
            v-if="gridTemplateRows.length != 1"
            @click="delRow(index)"
            plain
            size="mini"
            icon="el-icon-minus"
    ...
    <div
      class="grid-wrapper"
      :style="{ gridTemplateColumns: gridTemplateColumnsCss, gridTemplateRows: gridTemplateRowsCss, gridGap: gridGapCss }"
    >
      <div
        class="grid-item"
    ...
    <section class="property-value" v-if="choosenGridItem">
      ...
      <div class="value-set">
        <div class="key-value w-50">
          ...
          <el-input-number
            :min="1"
            :max="choosenGridItem.gridColumnEnd - 1"
            size="mini"
            v-model="choosenGridItem.gridColumnStart"
          ></el-input-number>
        </div>
        <div class="key-value w-50">
          <span class="key">grid-column-end:&nbsp;&nbsp;</span>
          <el-input-number
            :min="choosenGridItem.gridColumnStart + 1"
            :max="gridTemplateColumns.length + 1"
            size="mini"
            v-model="choosenGridItem.gridColumnEnd"
          ></el-input-number>
        </div>
      </div>
      <div class="language-css extra-class">
      ...
      <div class="value-set">
        <div class="key-value w-50 ">
          ...
          <el-input-number
            :min="1"
            :max="choosenGridItem.gridRowEnd - 1"
            size="mini"
            v-model="choosenGridItem.gridRowStart"
          ></el-input-number>
        </div>
        <div class="key-value w-50">
          <span class="key">grid-row-end:&nbsp;&nbsp;</span>
          <el-input-number
            :min="choosenGridItem.gridRowStart + 1"
            :max="gridTemplateRows.length + 1"
            size="mini"
            v-model="choosenGridItem.gridRowEnd"
          ></el-input-number>
        </div>
      </div>
    </section>
...
export default {
  ...
  computed: {
    gridTemplateColumnsCss() {
      return this.gridTemplateColumns.map(v => (v.value ? `${v.value}${v.unit}` : "")).join(" ");
    },
    gridTemplateRowsCss() {
      return this.gridTemplateRows.map(v => (v.value ? `${v.value}${v.unit}` : "")).join(" ");
    },
    gridGapCss() {
  ...
  methods: {
    ...
    addGridItem() {
      let color =
        this.gridItem.length >= this.colors.length
          ? this.colors[this.gridItem.length % this.colors.length]
          : this.colors[this.gridItem.length];
      this.gridItem.push({
    ...
    setRepeat(arr) {
      ...
      return newArr.map(v => (v.length == 1 ? v[0] : `repeat(${v.length}, ${v[0]})`)).join(" ");
    },
    updateColumnsCss() {
      this.$refs["gridTemplateColumns"].innerHTML = Prism.highlight(
        `grid-template-columns: ${this.setRepeat(this.gridTemplateColumnsCss.split(" "))};`,
        Prism.languages.css
      );
    },
    updateRowsCss() {
      this.$refs["gridTemplateRows"].innerHTML = Prism.highlight(
        `grid-template-rows: ${this.setRepeat(this.gridTemplateRowsCss.split(" "))};`,
        Prism.languages.css
      );
    },
    updateGaps() {
      this.$refs["gridcolumnGaps"].innerHTML = Prism.highlight(`grid-gap: ${this.gridGapCss};`, Prism.languages.css);
    },
    updateItemColumn() {
      setTimeout(() => {
        this.$refs["itemColumn"].innerHTML = Prism.highlight(`grid-column: ${this.itemColumn};`, Prism.languages.css);
      });
    },
    updateItemRow() {
      setTimeout(() => {
        this.$refs["itemRow"].innerHTML = Prism.highlight(`grid-row: ${this.itemRow};`, Prism.languages.css);
      });
    },
    updateCss() {
      this.updateColumnsCss();
